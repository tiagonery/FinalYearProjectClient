...
class FriendsListAdapter extends ArrayAdapter<User> {

    private List<Friendship> friendshipList;

    public FriendsListAdapter(Context context, List<User> users, List<Friendship> friendshipList){
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            switch (which){
                                case DialogInterface.BUTTON_POSITIVE:
                                    ((FriendsFragment)((HomeActivity)getContext()).getAdapter().getItem(2)).acceptFriendship(friendship);
                                    break;

                                case DialogInterface.BUTTON_NEGATIVE:
                                    ((FriendsFragment)((HomeActivity)getContext()).getAdapter().getItem(2)).refuseFriendship(friendship);
                                    break;
                            }
                        }
                    };
