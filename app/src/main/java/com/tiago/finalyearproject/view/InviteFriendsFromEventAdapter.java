...
class InviteFriendsFromEventAdapter extends ArrayAdapter<User> {


    private CheckBox[] checkBoxArray;
    private List<UserEvent> userEventList;
    private UserEvent.UserEventState mainUserEventState;

    public InviteFriendsFromEventAdapter(Context context, List<User> users, List<UserEvent> userEventList, UserEvent.UserEventState currentUserEventState){

        super(context, R.layout.add_friends_row_layout, users);
        this.userEventList = userEventList;
        checkBoxArray = new CheckBox[users.size()];
        mainUserEventState = currentUserEventState;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        View theView = theInflater.inflate(R.layout.invite_friends_event_row_layout, parent, false);
        ...
        CheckBox cBox = (CheckBox) theView.findViewById(R.id.checkbox_add_friends_event);
...
//        }
        ...
        UserEvent.UserEventState state = getUserEvent(userEventList, user.getFacebookId()).getState();
        if(state !=null || mainUserEventState!= UserEvent.UserEventState.OWNER){
            cBox.setVisibility(View.INVISIBLE);
        }
        // Get the TextView we want to edit
        TextView theTextView = (TextView) theView.findViewById(R.id.friendNameEventTextView);

        // Put the next TV Show into the TextView
        theTextView.setText(user.generateFullName());

        // Get the ImageView in the layout
        ImageView theImageView = (ImageView) theView.findViewById(R.id.friendPictureEventImageView);
